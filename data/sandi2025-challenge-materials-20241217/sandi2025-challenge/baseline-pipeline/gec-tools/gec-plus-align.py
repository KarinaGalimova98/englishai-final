...
def correct_grammar(input_texts, tokenizer, model, device, batch_size=32, num_return_sequences=1):
    ...
    for i in range(0, len(input_texts), batch_size):
        ...
        batch = tokenizer(batch_texts, padding=True, truncation=True, max_length=128, return_tensors="pt").to(device)
        translated = model.generate(**batch, max_new_tokens=128, num_beams=5, num_return_sequences=num_return_sequences)
