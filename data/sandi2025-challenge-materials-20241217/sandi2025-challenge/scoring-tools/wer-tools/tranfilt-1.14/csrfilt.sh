...
EXE_DIR=/scratches/dialfs/mvse/MGB-3/scoring/tranfilt-1.14
DeleteHyphens=false
UpCase=true;
ExtASCII=false;
...
if test "$DeleteHyphens" = "true" ; then
    ...
    # code amended by K Knill 6/Aug/2024 to prevent splitting of fileIDs    
    if test "$inputtype" = "ctm" ; then 
        filt_com="$filt_com | awk -f ./scoring-tools/wer-tools/tranfilt-1.14/ctm-hyphen.awk "
    else
        filt_com="$filt_com | perl -pe 's/([^ \(0-9])-([^ \)0-9])/\$1 \$2/g; s/([^\(\) 0-9])-([^\(\) 0-9])/\$1 \$2/g'"
    fi
fi
