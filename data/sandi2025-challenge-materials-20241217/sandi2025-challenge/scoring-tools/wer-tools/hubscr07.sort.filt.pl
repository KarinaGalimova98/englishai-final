...
#
my $Version = "0.7"; 
my $Usage="hubscr07.pl [ -R -v -L LEX ] [ -M LM | -w WWL ] -g glm -l LANGOPT -h HUBOPT -r ref [-k] hyp1 hyp2 ...\n".
"Version: $Version\n".
...
#############     Set all Global variables         #############
    ...
    ### empty.
    my $SCTK = "./scoring-tools/wer-tools/sctk-2.4.9";
    my $SCLITE = "";
    my $SC_STATS = "";
    ### Installation directory for the tranfilt package.   If the package's
    ### executables are accessible via your path, this variable may remain 
    ### empty.
    my $TRANFILT="./scoring-tools/wer-tools/tranfilt-1.14";
    my $CSRFILT="";
    my $DEF_ART="";
    my $GLM = "";
    my $ACOMP = "";
    ...
    my $SLM_LM = "";
    my $WWL = "";
    my $SortCTM = 1; # 0: no sort/keep input CTM order, 1: sort/not keep order
#######         End of Globals         #########
################################################
...
sub ProcessCommandLine{
    ...
    getopts('vRl:h:r:g:L:n:e:M:w:k');

    # $SortCTM = defined($main::opt_k) ? 1 : 0;
    if (defined($main::opt_k)) {    $SortCTM = 0; $main::opt_k = 0;}  # 0: no sort/keep input CTM order, 1: sort/not keep order
...
sub VerifyResources{
	...
	$SCLITE = "$SCTK/bin/sclite";
	$SC_STATS = "$SCTK/bin/sc_stats";
    }
...
sub FilterFile{
	...
	if ($format eq "ctm"){ # add an option o to choose sort/order or not
        # $sort_com = $SortCTM ? "sort +0 -1 +1 -2 +2nb -3" : "cat";
        # Determine the sort command based on format and sorting option
        if ($SortCTM == 1) {
