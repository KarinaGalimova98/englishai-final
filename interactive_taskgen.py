...
def get_interactive_task():
    ...
    prompt1 = (
        ...
        f"IMPORTANT: Output MUST be valid HTML.\n"
        f"- For each blank, insert the following HTML field directly into the text: {input_html}\n"
        f"- Do NOT use underscores, dots, or plain text like \"______\" or \"......\"\n"
        f"- Do NOT use Markdown, code blocks, or any formatting syntax — only valid HTML output.\n"
        f"- Do NOT include the correct answers in the text.\n"
        f"- At the end of the HTML block, append:\n"
        f"<script type='application/json' id='answers'>[\"correct1\", \"correct2\", ...]</script>\n"
        f"REQUIRED: If you don’t follow this structure exactly, the output will be rejected.\n"
