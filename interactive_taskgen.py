...
def get_interactive_task():
    ...
    prompt1 = (
        ...
        f"You are generating interactive Cambridge exam tasks for an educational website."
        f"The HTML is inserted directly into a live web page."
        f"Any mismatch in structure will break the frontend."
        f"Generate a {task_type} task for the {exam} exam."
        f"STRICT INSTRUCTIONS:\n\n"
        f"1. Output MUST be valid raw HTML. Do NOT use triple backticks, markdown, or code blocks or prompt.\n\n"
        f"2. Each blank MUST be replaced by this HTML input field:\n"
        f"   {input_html}\n"
        f"   Increment the name by 1 for each gap (name=\"1\", \"2\", ..., \"N\").\n\n"
        f"3. After the HTML block, add this EXACT line (no changes):\n"
        f"   <script type=\"application/json\" id=\"answers\">[\"answer1\", \"answer2\", ..., \"answerN\"]</script>\n\n"
        ...
        f"\n6. The number of <input> fields in the HTML MUST be exactly {num_gaps}, and there MUST be exactly {num_gaps} answers.\n"
        f"7. DO NOT OMIT ANY ANSWERS. If you skip or forget even one, the task will BREAK.\n"
        f"8. If the number of <input> fields and answers does NOT match, REGENERATE until it is correct.\n"
        f"Topic: {topic}."
